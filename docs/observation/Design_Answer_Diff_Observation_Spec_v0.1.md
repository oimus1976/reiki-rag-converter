...
※ Reference Diff は、参照表現の「出現有無・集合差」のみを観測対象とする。
章構成の説明や概要文（章サマリ）に起因する参照表現の増減は、
文脈的・説明的差分として扱い、評価や正誤判断を行わない。
...
章構成の説明文や概要記述に含まれる参照表現
（例：「第1章（第1条〜第5条）」）は、
正規表現上は検出対象となりうる。

ただし、本仕様ではそれらを
「説明表現に付随する参照差」として扱い、
参照の正確性・網羅性・過不足についての判断は行わない。
...
#### 10.3.1 入力ディレクトリ（論理構造）

compare_answers.py が扱う論理的な入力構造は、以下とする。

```text
<root>
└─ <ordinance_id>/
   └─ <question_id>/
      └─ <question_id>_answer.md
```

ここでいう `<root>` は、
zip 展開後の任意のディレクトリ階層を含みうる論理ルートであり、
zip 直下のディレクトリ名や中間階層の名称は意味を持たない。

---

#### 10.3.2 zip 展開後のディレクトリ走査ルール

compare_answers.py は、
zip 展開後または展開済みディレクトリに対して
**再帰的にディレクトリ走査**を行い、
以下の条件を満たすファイルを探索する。

- ファイル名が `<question_id>_answer.md` の形式であること
- 親ディレクトリ名が `<question_id>` と一致すること
- その親の親ディレクトリ名を `<ordinance_id>` とみなせること

zip 直下に以下のような余分な階層が存在してもよい。
...
compare_answers.py は、
これらの中間ディレクトリ名を解釈せず、
**`(ordinance_id, question_id)` を一意に識別できる構造のみを信頼する。**

---

#### 10.3.3 入力対応付けと欠落時の扱い

html-input / markdown-input の入力構造について、
以下を前提条件とする。

- `(ordinance_id, question_id)` の集合は
  **両入力で完全一致していることを期待する**
- 両側に存在する組のみを AnswerPair として扱う
- 片側にのみ存在する組は：
  - 観測対象からは除外する
  - エラーとして記録する（警告扱い）

欠落は、
入力生成やテスト工程の問題を検知するための情報であり、
差分観測のロジックには影響させない。
