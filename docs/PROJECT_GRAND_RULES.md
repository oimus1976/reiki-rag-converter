...
ChatGPT は、ユーザーが `git commit` を実行しようとする直前のタイミングで、
**プロジェクトファイル（PROJECT_STATUS / PROJECT_GRAND_RULES / requirements / test_plan / Design_* など）に変更の可能性があるかを自動判定し、必要に応じて以下をリマインドする義務を負う。**
...
```planetext
⚠ プロジェクトファイルに変更が生じています。
　コミットには以下のファイルの追加/更新が必要です：
　- docs/PROJECT_GRAND_RULES.md
　- docs/test_plan.md
　- docs/requirements.md
　…
必要なら git add → git commit → git push を案内します。
```

#### ❗ 運用ポリシー

- ChatGPT は **commit 作業を“開始したい”サイン**
  （例：`git status` が貼られる、ユーザーが commit 文を求める）
  を検知したら必ずチェックする。
- “変更すべき文書があるのに commit を促す”行為は禁止。
